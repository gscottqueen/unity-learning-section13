...
public class DragInteractable : XRBaseInteractable
{
  ...
  void StartDrag()
  {
    ...
    m_drag = StartCoroutine(CalculatedDrag());
    onDragStart?.Invoke();
  }
  ...
  IEnumerator CalculatedDrag()
  {
    Debug.Log("Calculated Drag");
    ...
    while(m_interactor != null)
    {
      ...
      Vector3 interactorLocalPositoin = startDragPosition.parent.InverseTransformPoint(m_interactor.transform.position);
      // project the interactor position onto the line
      Vector3 projectedPoint = Vector3.Project(interactorLocalPositoin, line.normalized);
